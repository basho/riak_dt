%% -------------------------------------------------------------------
%%
%% riak_dt_map2: OR-Set schema based multi CRDT container
%%
...
value({_Clock, Values, _Deferred}) ->
    orddict:fold(fun({Name, Type}, {_Dots, CRDT}, Acc) ->
...
get({_Name, Type}=Field, Fields, Clock) ->
    CRDT = case orddict:find(Field, Fields) of
               {ok, {_Dots, CRDT0}} ->
                   CRDT0;
               error ->
                   Type:new()
...
get_entry({_Name, Type}=Field, Fields, Clock) ->
    {Dots, CRDT} = case orddict:find(Field, Fields) of
                       {ok, Entry} ->
                           Entry;
                       error ->
                           {[], Type:new()}
...
apply_ops([{update, {_Name, Type}=Field, Op} | Rest], Dot, {Clock, Values, Deferred}, Ctx) ->
    ...
    case Type:update(Op, Dot, CRDT, Ctx) of
        {ok, Updated0} ->
            ...
            NewValues = orddict:store(Field, {[Dot], Updated}, Values),
...
remove_field(Field, {Clock, Values, Deferred0}, Ctx) ->
    ...
    NewValues = case ctx_rem_field(Field, Values, Ctx, Clock) of
                    ...
                    CRDT ->
                        orddict:store(Field, CRDT, Values)
                end,
...
merge({LHSClock, LHSEntries, LHSDeferred}, {RHSClock, RHSEntries, RHSDeferred}) ->
    ...
    Fields = lists:umerge(orddict:fetch_keys(LHSEntries), orddict:fetch_keys(RHSEntries)),
    Entries = lists:foldl(fun({_Name, Type}=Field, Acc) ->
                                  ...
                                  case keep_dots(LHSDots, RHSDots, LHSClock, RHSClock) of
                                      ...
                                      Dots ->
                                          ...
                                          orddict:store(Field, {Dots, Type:parent_clock(?FRESH_CLOCK, CRDT)}, Acc)
                                  end
                          end,
                          orddict:new(),
                          Fields),
