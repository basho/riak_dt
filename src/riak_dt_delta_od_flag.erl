...
-export([new/0, value/1, value/2, update/3, update/4, delta_update/3]).
...
-type od_flag_delta() :: {riak_dt_vclock:vclock(), [riak_dt:dot()], deferred()}.
...
delta_update(enable, Dot, {Clock, _Dots, Deferred}) when is_tuple(Dot) ->
    NewClock = riak_dt_vclock:merge([[Dot], Clock]),
    {ok, {NewClock, [Dot], Deferred}};
delta_update(enable, Actor, {Clock, _Dots, Deferred}) ->
    NewClock = riak_dt_vclock:increment(Actor, Clock),
    Dot = [{Actor, riak_dt_vclock:get_counter(Actor, NewClock)}],
    {ok, {NewClock, Dot, Deferred}};
...
%% NewDots contains dots that are not yet visible in the given context
...
defer_disable(Clock, Ctx, Deferred) ->
    io:format("Defer disable ~p ~p",[Clock, Ctx]),
