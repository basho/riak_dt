...
-spec delta_update(orswot_op(), actor() | dot(), delta_orswot(),
                   riak_dt:context()) ->
...
delta_update({remove, Elem}, _Actor, {_, Entries, _, _}=ORSet0) ->
    case remove_elem(?DICT:find(Elem, Entries), Elem, ORSet0) of
        {ok, ORSet} ->
            {ok, Delta} = delta_remove_elem(Elem, ORSet0, new()),
            {ok, {ORSet, Delta}};
        ErrorTup ->
            ErrorTup
    end;
...
delta_update({remove, Elem}, _Actor, {_, Entries, _, _}=ORSet0, Ctx) ->
    case remove_elem(?DICT:find(Elem, Entries), Elem, ORSet0, Ctx) of
        {ok, ORSet} ->
