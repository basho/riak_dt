...
apply_ops([], _Dot, Clock, Values) ->
    {ok, {Clock, Values}};
apply_ops([{update, {Name, Type}=Field, Op} | Rest], Dot, Clock, Values) ->
    ...
    case Type:update(Op, Dot, CRDT) of
        {ok, Updated} ->
            NewValues = ordsets:add_element({Field, Updated, Dot, true}, Values),
