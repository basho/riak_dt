...
remove({Replica, Field}, ReplicaData) ->
    ...
    %% don't change state)
    Map2 = ignore_preconerror_remove(Field, ReplicaData, Map),
...
update_args(#state{replicas=Replicas, replica_data=ReplicaData, counter=Cnt}) ->
    [
     ?LET(Field, gen_field(), {Field, gen_field_op(Field)}), %% An op to perform
     elements(Replicas), %% A replica to update
     ReplicaData,
     Cnt
    ].
                ...
                {MapValue, ModelValue} = case Replicas of
                                             ...
                                             _L ->
                                                 ...
                                                 {Map, Model} = lists:foldl(fun(Actor, {M, Mo}) ->
                                                                            ...
                                                                            Replicas),
                                                 %% io:format("~nXXXX FINAL XXXX~nMap:::: ~p~nModel:::: ~p~n", [Map, Model]),
...
model_add_field({_Name, Type}=Field, Cnt, {Adds, Removes}) ->
    ...
    {ok, {sets:add_element(ToAdd, Adds), Removes}}.
...
model_update_field({Name, Type}=Field, Op, Actor, Cnt, {Adds, Removes}=Model) ->
    ...
    CRDT = lists:foldl(fun({{FName, FType}, Value, _X}, Acc) when FName == Name,
                       ...
                       Type:new(),
                       sets:to_list(InMap)),
    case Type:update(Op, Actor, CRDT) of
        {ok, Updated} ->
